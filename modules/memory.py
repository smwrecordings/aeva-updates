...
class AevaMemory:
    ...
    def _load(self):
        if os.path.exists(self.path):
            try:
                ...
                print(
                    "[AevaMemory] Warning: Failed to load memory. Initializing empty store.")
                self.memory = []
    ...
    def store(self, user_input, context=None, emotion=None):
        entry = {
            ...
            "emotion": emotion or self._get_mood(),
            "persona": self._get_persona()
        }
        self.memory.append(entry)
        self._save()
    ...
    def search(self, keyword):
        results = [e for e in self.memory if keyword.lower()
                   in json.dumps(e).lower()]
        print(
            f"[AevaMemory] Search for '{keyword}': {
    ...
    def _get_mood(self):
        try:
            return self.brain.emotions.get_current_state().get("mood", "neutral")
        except Exception:
            return "neutral"

    def _get_persona(self):
        try:
            return self.brain.persona.mood
        except Exception:
            return "unknown"
    ...
    dummy_brain = SimpleNamespace(
        emotions=SimpleNamespace(
            get_current_state=lambda: {
                "mood": "curious"}), persona=SimpleNamespace(
            mood="exploring"))
